...
export default function Services() {
  ...
  return (
    <section className='py-24 px-4 sm:px-6 lg:px-8 bg-white relative overflow-hidden'>
      ...
      <div className='absolute top-0 left-1/2 -translate-x-1/2 w-[800px] h-[800px] bg-linear-to-b from-green-50 to-transparent rounded-full blur-3xl opacity-50 -z-10' />
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 gap-8'>
          {services.map((service, index) => {
            ...
            return (
              <div
                key={index}
                className='group p-8 rounded-2xl border border-gray-200 hover:border-[#059669] bg-white hover:shadow-2xl transition-all duration-300 animate-in fade-in slide-in-from-bottom'
                style={{ animationDelay: `${index * 100}ms` }}
              >
                <div className='inline-flex p-4 rounded-xl bg-[#059669]/10 text-[#059669] mb-6 group-hover:scale-110 transition-transform duration-300'>
                  <Icon className='w-8 h-8' />
                </div>
                <h3 className='text-2xl font-bold text-gray-900 mb-4 group-hover:text-[#059669] transition-colors'>
                  {service.title}
                </h3>
